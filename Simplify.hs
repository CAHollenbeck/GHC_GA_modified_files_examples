...
import ByteCodeInstr ( vecprFtrs ) -- UoE
{-
...
idxDoubleA :: ArgInfoFeature -> Double
idxDoubleA a = fromIntegral (fromEnum a)

data ArgInfoFeature = 
  AINonTrivArg
  | AIValueArg
  | AITrivArg
  | NumAIFeatures deriving (Enum)

arginfoIdxs :: [ArgSummary] -> [Double] -> [Double]
arginfoIdxs [] al = al
arginfoIdxs (a:as) al = case a of
                          NonTrivArg -> arginfoIdxs as ([(idxDoubleA AINonTrivArg)] ++ al)
                          ValueArg   -> arginfoIdxs as ([(idxDoubleA AIValueArg)] ++ al)
                          TrivArg    -> arginfoIdxs as ([(idxDoubleA AITrivArg)] ++ al)
--------- UoE End

-- UoE: removed "interesting_cont" from being passed to completeCall
completeCall :: SimplEnv -> OutId -> SimplCont -> SimplM (SimplFloats, OutExpr)
completeCall env var cont
  | Just expr <- callSiteInline dflags var active_unf
                                lone_variable arg_infos simplify_ftrs interesting_cont
  ...
  where
    simplify_ftrs = -- (vecprFtrs (contIdxs cont cont_depth) (idxDoubleC NumContFeatures)) ++
                     cont_ftrs ++
                     arginfo_ftr_vec ++
                     [(fromIntegral n_val_args)] ++ [(fromIntegral lone_var_int)] -- ++ [(fromIntegral active_unf_int)] 
    -- cont_depth = 5 -- How many nodes to recur into for the SimplCont features
    cont_ftrs = (vecprFtrs cont_enums (idxDoubleC NumContFeatures))
    lone_var_int = getBool lone_variable
    active_unf_int = getBool active_unf
    arginfo_ftrs = arginfoIdxs arg_infos []
    arginfo_ftr_vec = vecprFtrs arginfo_ftrs (idxDoubleA NumAIFeatures)
    dflags = seDynFlags env
    (lone_variable, arg_infos, call_cont) = contArgs cont
    n_val_args       = length arg_infos
    cont_enums = ftrsCallContext env call_cont 10 -- UoE: this is now of type [Double] & represents the SimplCtxt enums
    interesting_cont = interestingCallContext env call_cont -- UoE: this is now of type [Double] & represents the SimplCtxt enums
