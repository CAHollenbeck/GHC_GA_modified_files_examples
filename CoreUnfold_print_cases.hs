...
-- Begin UoE
import ByteCodeInstr ( FunInlineFeature(..), vecprFtrs, funcIdxs, idxDoubleF )
import GAUtils
-- End UoE
...
--------- begin UoE sizeExpr
-- This was taken from exprSize in coreSyn/CoreStats.hs
-- Should roughtly calculate the size of the UnfWhen expr
uoeExprSize :: CoreExpr -> Int
-- ^ A measure of the size of the expressions, strictly greater than 0
-- Counts *leaves*, not internal nodes. Types and coercions are not counted.
uoeExprSize (Var _)         = 1
uoeExprSize (Lit _)         = 1
uoeExprSize (App f a)       = uoeExprSize f + uoeExprSize a
uoeExprSize (Lam b e)       = uoeBndrSize b + uoeExprSize e
uoeExprSize (Let b e)       = uoeBindSize b + uoeExprSize e
uoeExprSize (Case e b _ as) = uoeExprSize e + uoeBndrSize b + 1 + sum (map uoeAltSize as)
uoeExprSize (Cast e _)      = 1 + uoeExprSize e
uoeExprSize (Tick n e)      = uoeTickSize n + uoeExprSize e
uoeExprSize (Type _)        = 1
uoeExprSize (Coercion _)    = 1

uoeTickSize :: Tickish Id -> Int
uoeTickSize (ProfNote _ _ _) = 1
uoeTickSize _ = 1

uoeBndrSize :: Var -> Int
uoeBndrSize _ = 1

uoeBndrsSize :: [Var] -> Int
uoeBndrsSize = sum . map uoeBndrSize


uoeBindSize :: CoreBind -> Int
uoeBindSize (NonRec b e) = uoeBndrSize b + uoeExprSize e
uoeBindSize (Rec prs)    = sum (map uoePairSize prs)

uoePairSize :: (Var, CoreExpr) -> Int
uoePairSize (b,e) = uoeBndrSize b + uoeExprSize e

uoeAltSize :: CoreAlt -> Int
uoeAltSize (_,bs,e) = uoeBndrsSize bs + uoeExprSize e
--------- end Uoe sizeExpr 
    ...
    size_up (Case e _ _ alts)
        ...
        = let
            ...
            alts_size (SizeIs tot tot_disc tot_scrut)
                  ...
                  = SizeIs tot (unitBag (v, 38 + tot - max) -- nov2
...
callSiteInline :: DynFlags
               ...
               -> [Double]  -- UoE
               -> CallCtxt              -- True <=> continuation is interesting -- UoE: took this out to save time
...
callSiteInline dflags id active_unfolding lone_variable arg_infos cont_ftrs cont_info
  = case idUnfolding id of
      ...
      -- be a loop breaker  (maybe the knot is not yet untied)
        CoreUnfolding { uf_tmpl = unf_template
          ...
          | active_unfolding -> case guidance of
                      --             _ -> tryUnfolding dflags id lone_variable arg_infos cont_info unf_template is_wf is_exp guidance         
                                  UnfNever -> (traceInline dflags id str (text "UnfNever") Nothing)
                                  UnfWhen { ug_arity = uf_arity,
                                            ug_boring_ok = boring_ok,
                                            ug_unsat_ok = unsat_ok,
                                            ug_size = uf_size
                                             } -> case answer of
                                                     Just expr  -> if activ_bool 
                                                                      then trace ("1, 1, " ++ (show final_ftrs)) answer  
                                                                      else trace ("0, 1, " ++ (show final_ftrs)) answer -- (NN_no, GHC_yes ...
                                                     Nothing     -> if activ_bool
                                                                      then trace ("1, 0, " ++ (show final_ftrs)) answer -- (NN_yes, GHC_no ...
                                                                      else trace ("0, 0, " ++ (show final_ftrs)) answer  
                                                     _           -> trace "UW BIG PROBLEM" answer
                                                  --   tryUnfolding dflags id lone_variable
                                                  --     arg_infos cont_info unf_template
                                                  --     is_wf is_exp guidance
                                                  
                                                  --if activ_bool -- (enough_args && (boring_ok || some_benefit)) 
                                                  --          then traceInline dflags id str (mk_doc True empty True) (Just unf_template)
                                                  --          else traceInline dflags id str (mk_doc False empty False) Nothing 
                                                  where
                                                      answer = tryUnfolding dflags id lone_variable arg_infos cont_info unf_template is_wf is_exp guidance
                                                      some_benefit = calc_some_benefit uf_arity
                                                      enough_args = (n_val_args >= uf_arity) || (unsat_ok && n_val_args > 0)
                                                      activ_bool = activation_output > 0.5
                                                      activation_output = (activate model final_ftrs)
                                                      uf_ftrs = [0.0,
                                                                     (fromIntegral uf_size),
                                                                     (fromIntegral (getBool is_wf))]
                                                      final_ftrs = features ++ uf_ftrs
                                                      -- TODO added calc_some_benefit to see if it fixed linker error
                                  ...
                                  UnfIfGoodArgs { ug_size = uf_size }
                                           -> case answer of
                                                Just expr -> if activ_bool
                                                               then trace ("1, 1, " ++ (show final_ftrs)) answer -- (NN_yes, GHC_yes ...
                                                               else trace ("0, 1, " ++ (show final_ftrs)) answer -- (NN_no, GHC_yes ...
                                                Nothing   -> if activ_bool
                                                               then trace ("1, 0, " ++ (show final_ftrs)) answer -- (NN_yes, GHC_no ...
                                                               else trace ("0, 0, " ++ (show final_ftrs)) answer -- (NN_no, GHC_no ...
                                                _         -> trace "IGA BIG PROBLEM" answer
                                               where
                                                       answer = tryUnfolding dflags id lone_variable arg_infos cont_info unf_template is_wf is_exp guidance
                                                       activ_bool = activation_output > 0.5
                                                       activation_output = (activate model final_ftrs)
          ...
          where
            ...
            features = cont_ftrs ++
                    fn_ftrs
            lone_var_int = getBool lone_variable
            -- active_unf_int = getBool active_unfolding
            fn_ftrs = vecprFtrs (funcIdxs unf_template) (idxDoubleF NumFunFeatures)
            maybe_model = modelNN dflags 
            model = case maybe_model of
                      Nothing -> error "Error: Model not passed" -- Nothing -- Don't error if we're printing & not using the model
                      Just a  -> a
...
-- UoE : Just modified this to print out the feature vector if it inlines
tryUnfolding_trace :: DynFlags -> Id -> Bool -> [ArgSummary] -> CallCtxt
             -> CoreExpr -> Bool -> Bool -> UnfoldingGuidance
             -> [Double] -- UoE 
             -> Maybe CoreExpr
tryUnfolding_trace dflags id lone_variable
             arg_infos cont_info unf_template
             is_wf is_exp guidance ftr_vec
 = case guidance of
     UnfNever -> traceInline dflags id str (text "UnfNever") Nothing
     ...
     UnfWhen { ug_arity = uf_arity, ug_unsat_ok = unsat_ok, ug_boring_ok = boring_ok }
        ...
        -> trace (show (ftr_vec ++ [1])) (traceInline dflags id str (mk_doc some_benefit empty True) (Just unf_template))
        | otherwise
        -> trace (show (ftr_vec ++ [0])) (traceInline dflags id str (mk_doc some_benefit empty False) Nothing)
        where
          some_benefit = calc_some_benefit uf_arity
          enough_args = (n_val_args >= uf_arity) || (unsat_ok && n_val_args > 0)
     ...
     UnfIfGoodArgs { ug_args = arg_discounts, ug_res = res_discount, ug_size = size }
        ...
        -> trace (show (ftr_vec ++ [1])) (traceInline dflags id str (mk_doc some_benefit extra_doc True) (Just unf_template))
        | is_wf && some_benefit && small_enough
        -> trace (show (ftr_vec ++ [1])) (traceInline dflags id str (mk_doc some_benefit extra_doc True) (Just unf_template))
        | otherwise
        -> trace (show (ftr_vec ++ [0])) (traceInline dflags id str (mk_doc some_benefit extra_doc False) Nothing)
        where
...
computeDiscount dflags arg_discounts res_discount arg_infos cont_info
  ...
  = 13          -- Discount of 10 because the result replaces the call -- nov2 FI, default 10
                -- so we count 10 for the function itself

    + 11 * length actual_arg_discounts  -- nov2 AA, default 10
  ...
  where
    ...
    mk_arg_discount _        NonTrivArg = 14 -- nov2 NTA
    ...
    res_discount'
      ...
      = case cont_info of
           ...
           DiscArgCtxt -> 22 `min` res_discount
           RuleArgCtxt -> 24 `min` res_discount
           RhsCtxt     -> 54 `min` res_discount
           _           -> 35 `min` res_discount
