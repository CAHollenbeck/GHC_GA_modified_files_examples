...
data ContInlineFeature =
  ...
  | CStrictArg
  | CTickIt
  | NumContFeatures deriving (Enum)

{-
getContFeat :: SimplCont -> ContInlineFeature
getContFeat sc = case sc of
                  Stop {} -> CStop
                  CastIt {} -> CCastIt
                  ApplyToVal {} -> CApplyToVal
                  ApplyToTy {} -> CApplyToTy
                  Select {} -> CSelect
                  StrictBind {} -> CStrictBind
                  StrictArg {} -> CStrictArg
                  TickIt {} -> CTickIt
-}

idxDoubleC :: ContInlineFeature -> Double
idxDoubleC e = fromIntegral (fromEnum e)
...
ftrsCallContext :: SimplEnv -> SimplCont -> Int -> [Double]
-- See Note [Interesting call context]
ftrsCallContext env cont count
  = interesting cont count
  where
    interesting _ 0 = []
    interesting (Select { sc_cont = child_cont }) count
       | sm_case_case (getMode env) = [(idxDoubleC CSelect)] ++ interesting child_cont (count - 1) 
       | otherwise                  = [(idxDoubleC CSelect)] ++ interesting child_cont (count - 1)
       -- See Note [No case of case is boring]

    interesting (ApplyToVal { sc_cont = child_cont }) count = [(idxDoubleC CApplyToVal)] ++ interesting child_cont (count - 1)
        -- Can happen if we have (f Int |> co) y
        -- If f has an INLINE prag we need to give it some
        -- motivation to inline. See Note [Cast then apply]
        -- in CoreUnfold

    interesting (StrictArg { sc_cci = cci, sc_cont = child_cont }) count = [(idxDoubleC CStrictArg)] ++ interesting child_cont (count - 1)
    interesting (StrictBind { sc_cont = child_cont }) count        = [(idxDoubleC CStrictBind)] ++ interesting child_cont (count - 1)
    interesting (Stop _ cci) count           = [(idxDoubleC CStop)]
    interesting (TickIt _ k) count           = [(idxDoubleC CTickIt)] ++ interesting k (count - 1)
    interesting (ApplyToTy { sc_cont = k }) count  = [(idxDoubleC CApplyToTy)] ++ interesting k (count - 1)
    interesting (CastIt _ k) count           = [(idxDoubleC CCastIt)] ++ interesting k (count - 1)
        -- If this call is the arg of a strict function, the context
        -- is a bit interesting.  If we inline here, we may get useful
        -- evaluation information to avoid repeated evals: e.g.
        --      x + (y * z)
