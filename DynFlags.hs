...
data DynFlags = DynFlags {
  -- modelPath             :: String, -- UoE
  modelNN               :: Maybe NNLink', -- UoE: the actual model
  ghcMode               :: GhcMode,
  ghcLink               :: GhcLink,
  hscTarget             :: HscTarget,
...
-- See Note [Updating flag description in the User's Guide]
     DynFlags {
        -- modelPath               = "Something went wrong",
        modelNN                 = Nothing, -- UoE
        ghcMode                 = CompManager,
        ghcLink                 = LinkBinary,
...
-- UoE
  , make_ord_flag defGhcFlag "modelNN"
        (HasArg (\s -> upd (\d -> d { modelNN = getModelFromFlag s }
                   )))
-- UoE
...
-- UoE begin
--  , make_ord_flag defFlag "model-path"
--     (sepArg (\s d -> d { modelPath = Just s })) 
-- UoE end
